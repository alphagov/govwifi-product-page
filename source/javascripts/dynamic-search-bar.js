var filterInput = document.getElementById('search-table');
filterInput.addEventListener('keyup', filterResults);

function filterResults() {
  var filterValue = filterInput.value.toUpperCase();
  var rows = document.querySelectorAll('#results');

  for(var i = 0; i < rows.length; i++) {
    var text = rows[i].querySelectorAll('.govuk-table__cell')[0].innerHTML.toUpperCase();
